...
public class PokemonService {
    ...
    public void create(Pokemon pokemon) {
        ...
        typeRepository.find(pokemon.getType().getTypeName()).ifPresent(type -> type.getPokemons().add(pokemon));
    }
    ...
    public Optional<Pokemon> findForType(String var1, String typeName) {
        Optional<Type> type = typeRepository.find(typeName);
        if(type.isEmpty()) {
            return Optional.empty();
        } else {
            return pokemonRepository.findByVarAndType(var1, type.get());
        }
    }
}
